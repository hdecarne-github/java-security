 ...
 * 256 bit AES cipher.
 */
class AES256Cipher extends Cipher {
	...
	AES256Cipher(SecretKeySpec secretKeySpec, byte[] salt, int saltOffset, int saltLength) {
		this.secretKeySpec = secretKeySpec;
		this.salt = new byte[saltLength];
		System.arraycopy(salt, saltOffset, this.salt, 0, saltLength);
	}

	public static byte[] generateSecret() throws GeneralSecurityException {
		LOG.info("Generating AES-256 cipher secret...");
	...
	public static AES256Cipher getInstance(byte[] secret) {
		if (secret.length < 1 + SALT_LENGTH || secret[0] != ID) {
			throw new IllegalArgumentException("Invalid AES256 cipher secret");
		}

		SecretKeySpec secretKeySpec = new SecretKeySpec(secret, 1 + SALT_LENGTH, secret.length - 1 - SALT_LENGTH,
				KEY_ALG);

		return new AES256Cipher(secretKeySpec, secret, 1, SALT_LENGTH);
	}
